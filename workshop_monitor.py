...
def workshop_monitor():
    while True:
        try:
            soupOne = BeautifulSoup(requests.get(url, headers=headers).content, 'html.parser')
            currentTags = []
            for tag in soupOne.find_all(class_="ugc"):
                currentTags.append(tag.get('data-publishedfileid'))

            time.sleep(60)

            soupTwo = BeautifulSoup(requests.get(url, headers=headers).content, 'html.parser')
            newTags = []
            for tag in soupTwo.find_all(class_="ugc"):
                newTags.append(tag.get('data-publishedfileid'))

            urlList, nameList = [], []
            if currentTags != newTags:
                [i for i in currentTags if not i in newTags or newTags.remove(i)]
                workshop = soupTwo.find("div", {"class": "workshopBrowseItems"})
                for tag in newTags:
