...
def send_messages():
    ...
    for name, userID, lang in zip(data['Name'], data['UserID'], data['Language']):
        if lang in CIS_lang_codes:
            ...
            bot.send_message(userID, text, parse_mode='Markdown', reply_markup=buttons.markup_ru)
            time.sleep(5)
        else:
            with open(config.USER_EN_MESSAGE_FILE_PATH, 'r') as f:
                text = f.read().format(username = name)
            bot = telebot.TeleBot(config.BOT_TOKEN)
            bot.send_message(userID, text, parse_mode='Markdown', reply_markup=buttons.markup_en)
            time.sleep(5)
